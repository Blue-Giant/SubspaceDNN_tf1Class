    ...
    with tf.device('/gpu:%s' % (R['gpuNo'])):
        with tf.compat.v1.variable_scope('vscope', reuse=tf.compat.v1.AUTO_REUSE):
            ...
            if R['opt2loss_bd'] == 'unified_boundary':
                loss_bd2left = model.loss_bd2NormalAddScale(XYZ_left, Ubd_exact=u00, alpha=R['contrib2scale'])
                loss_bd2right = model.loss_bd2NormalAddScale(XYZ_right, Ubd_exact=u00, alpha=R['contrib2scale'])
                loss_bd2bottom = model.loss_bd2NormalAddScale(XYZ_bottom, Ubd_exact=u00, alpha=R['contrib2scale'])
                loss_bd2top = model.loss_bd2NormalAddScale(XYZ_top, Ubd_exact=u00, alpha=R['contrib2scale'])
                loss_bd2front = model.loss_bd2NormalAddScale(XYZ_front, Ubd_exact=u00, alpha=R['contrib2scale'])
                loss_bd2behind = model.loss_bd2NormalAddScale(XYZ_behind, Ubd_exact=u00, alpha=R['contrib2scale'])
                Loss_bd2NNs = bd_penalty * (loss_bd2left + loss_bd2right + loss_bd2bottom + loss_bd2top +
                                            loss_bd2front + loss_bd2behind)
            else:
                loss_bd2Normal_left = model.loss2Normal_bd(XYZ_left, Ubd_exact=u00)
                loss_bd2Normal_right = model.loss2Normal_bd(XYZ_right, Ubd_exact=u00)
                loss_bd2Normal_bottom = model.loss2Normal_bd(XYZ_bottom, Ubd_exact=u00)
                loss_bd2Normal_top = model.loss2Normal_bd(XYZ_top, Ubd_exact=u00)
                loss_bd2Normal_front = model.loss2Normal_bd(XYZ_front, Ubd_exact=u00)
                loss_bd2Normal_behind = model.loss2Normal_bd(XYZ_behind, Ubd_exact=u00)
                ...
                loss_bd2Scale_left = model.loss2Scale_bd(XYZ_left, alpha=using_scale2boundary)
                loss_bd2Scale_right = model.loss2Scale_bd(XYZ_right, alpha=using_scale2boundary)
                loss_bd2Scale_bottom = model.loss2Scale_bd(XYZ_bottom, alpha=using_scale2boundary)
                loss_bd2Scale_top = model.loss2Scale_bd(XYZ_top, alpha=using_scale2boundary)
                loss_bd2Scale_front = model.loss2Scale_bd(XYZ_front, alpha=using_scale2boundary)
                loss_bd2Scale_behind = model.loss2Scale_bd(XYZ_behind, alpha=using_scale2boundary)
            ...
            UNN_Normal2test, UNN_Scale2test, UNN2test = model.evalue_MscaleDNN(X_points=XYZ_it, alpha=R['contrib2scale'])
