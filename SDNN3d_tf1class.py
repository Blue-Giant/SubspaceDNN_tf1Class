        ...
        with tf.compat.v1.variable_scope('vscope', reuse=tf.compat.v1.AUTO_REUSE):
            ...
            if R['PDE_type'] == 'general_Laplace':
                UNN2train, Loss_it2NNs, UNN_dot_UNN = sd2nn.loss_it2Laplace(
                    X=XYZ_it, fside=f, loss_type=R['loss_type'], alpha=R['contrib2scale'],
                ...
                UNN2train, Loss_it2NNs, UNN_dot_UNN = sd2nn.loss_it2pLaplace(
                    X=XYZ_it, Aeps=A_eps, fside=f, loss_type=R['loss_type'], alpha=R['contrib2scale'],
                    opt2orthogonal=R['opt2orthogonal'])
            elif R['PDE_type'] == 'Possion_Boltzmann':
                UNN2train, Loss_it2NNs, UNN_dot_UNN = sd2nn.loss_it2Possion_Boltzmann(
                    X=XYZ_it, Aeps=A_eps, fside=f, Kappa_eps=kappa, loss_type=R['loss_type'], alpha=R['contrib2scale'],
                    opt2orthogonal=R['opt2orthogonal'])
            ...
            if R['opt2loss_bd'] == 'unified_boundary':
                ...
                loss_bd2Scale_left = sd2nn.loss2Scale_bd(XYZ_left, alpha=R['contrib2scale'])
                loss_bd2Scale_right = sd2nn.loss2Scale_bd(XYZ_right, alpha=R['contrib2scale'])
                loss_bd2Scale_bottom = sd2nn.loss2Scale_bd(XYZ_bottom, alpha=R['contrib2scale'])
                loss_bd2Scale_top = sd2nn.loss2Scale_bd(XYZ_top, alpha=R['contrib2scale'])
                loss_bd2Scale_front = sd2nn.loss2Scale_bd(XYZ_front, alpha=R['contrib2scale'])
                loss_bd2Scale_behind = sd2nn.loss2Scale_bd(XYZ_behind, alpha=R['contrib2scale'])
